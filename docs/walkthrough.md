...
### 为什么补 typed stub / skeleton

早期版本需要调用方手写 `serial_id`、手动序列化请求、手动判断响应类型。现在 `.proto` 中声明 `CorpCronRpc` service，并通过生成脚本输出 `CorpCronRpcStub` 和 `CorpCronRpcSkeleton`。这样调用方可以写 `stub.SubmitTask(req, resp)`，服务端也具备把强类型 handler 包装成统一 RPC 分发函数的基础，更接近真实 RPC 框架的 IDL 驱动模式。

### 为什么加 interceptor 链

RPC 请求进入业务 handler 前会先经过 interceptor 链。当前默认链路负责异常兜底、请求/响应结构化日志和 RPC 成功失败指标统计；`TcpServer` 只保留连接、收发和解包职责。这样后续继续加入 trace、审计、限流或权限策略时，可以在请求管线中扩展，而不是把横切逻辑散到网络层和业务 handler 里。

### 为什么做简化 streaming

完整 gRPC streaming 需要 HTTP/2、多路复用、流控和取消传播。这个项目选择在现有 TCP 长连接多帧协议上实现一个简化版 server-side streaming：一个请求可以对应多帧响应，最后一帧用 `end_of_stream` 标记结束。这样可以展示流式 RPC 的基本语义，又不会把项目复杂度拉到不可控。
...
- 超时和取消：调度器通过 `scheduler.task_timeout_ms` 生成 RPC deadline，任务取消或服务停止会通过 cancellation token 打断等待中的 RPC 调用，避免任务长期占用线程池工作线程。
...
> 下一步我会继续补慢查询统计、故障注入测试和更细的权限模型，把存储抖动、坏节点恢复和任务执行隔离这几类生产问题验证得更充分。
