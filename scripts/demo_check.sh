...
echo "== Binaries =="
for bin in build/corpcron_server build/client/corpcron_client build/test_submit_client build/bench_client build/corpcron_rpc_client_test build/corpcron_rpc_client_pool_test build/corpcron_rpc_service_bindings_test; do
    if [[ -x "${bin}" ]]; then
        echo "OK   ${bin}"
    else
        echo "MISS ${bin}"
