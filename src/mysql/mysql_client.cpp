...
bool task_exists(sql::Connection& conn, const std::string& id) {
    std::unique_ptr<sql::PreparedStatement> statement(
        conn.prepareStatement("SELECT 1 FROM tasks WHERE id=? LIMIT 1"));
    statement->setString(1, id);
    std::unique_ptr<sql::ResultSet> result(statement->executeQuery());
    return result->next();
}

bool task_exists_and_is_not_running(sql::Connection& conn, const std::string& id) {
    std::unique_ptr<sql::PreparedStatement> statement(
        conn.prepareStatement("SELECT 1 FROM tasks WHERE id=? AND status<>? LIMIT 1"));
    statement->setString(1, id);
    statement->setInt(2, TASK_RUNNING);
    std::unique_ptr<sql::ResultSet> result(statement->executeQuery());
    return result->next();
}
...
std::string local_timezone_offset() {
    ...
    std::tm local_tm{};
    if (localtime_r(&now, &local_tm) == nullptr) return "+00:00";

    char compact_offset[8]{};
    if (std::strftime(compact_offset, sizeof(compact_offset), "%z", &local_tm) == 0) {
        return "+00:00";
    }
    std::string offset(compact_offset);
    if (offset.size() == 5 && (offset.front() == '+' || offset.front() == '-')) {
        offset.insert(3, ":");
        return offset;
    }
    return "+00:00";
...
MySQLClient::MySQLClient(const std::string& host, int port,
    ...
    if (options_.connect_timeout_sec <= 0) options_.connect_timeout_sec = 3;
    if (options_.read_timeout_sec <= 0) options_.read_timeout_sec = 5;
    if (options_.write_timeout_sec <= 0) options_.write_timeout_sec = 5;
}
    ...
    try {
        if (slot.conn) {
            try {
                ...
                recordSqlError("MySQL close before reconnect", e);
            }
            slot.conn.reset();
        }
    ...
    for (auto& slot : slots_) {
        ...
        if (slot->conn) {
            try {
                ...
                recordSqlError("MySQL disconnect", e);
            } catch (const std::exception& e) {
                LOG_ERROR(std::string("MySQL disconnect error: ") + e.what());
            }
            slot->conn.reset();
        }
    }
...
bool MySQLClient::updateTaskDefinition(const TaskMeta& task) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> pstmt(lease.conn->prepareStatement(
            "UPDATE tasks SET cron_expr=?, params=?, handler=?, status=?, next_run_at=?, "
            "last_run_at=?, retry_count=?, max_retries=? WHERE id=? AND status<>?"));
        ...
        pstmt->setInt(10, TASK_RUNNING);
        return pstmt->executeUpdate() > 0 || task_exists_and_is_not_running(*lease.conn, task.id);
    } catch (sql::SQLException &e) {
...
std::vector<TaskMeta> MySQLClient::getStaleRunningTasks(int stale_after_sec, size_t limit) {
    auto lease = acquireConnection();
    std::vector<TaskMeta> tasks;
    if (!lease.valid()) return tasks;
    if (stale_after_sec < 0) stale_after_sec = 0;
    try {
        std::unique_ptr<sql::PreparedStatement> pstmt(lease.conn->prepareStatement(
            std::string("SELECT ") + kTaskSelectFields +
        ...
        pstmt->setInt(2, -stale_after_sec);
        pstmt->setUInt(3, static_cast<unsigned int>(limit));
        std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
        while (res->next()) {
            tasks.push_back(task_from_result(*res));
        }
    } catch (sql::SQLException& e) {
        recordSqlError("getStaleRunningTasks", e);
    }
    return tasks;
}
...
bool MySQLClient::recoverTaskExecution(const std::string& id,
                                       const std::string& execution_id) {
    auto lease = acquireConnection();
    if (!lease.valid()) return false;
    try {
        std::unique_ptr<sql::PreparedStatement> pstmt(lease.conn->prepareStatement(
            "UPDATE tasks SET status=?, current_execution_id=NULL, running_node=NULL, "
            "started_at=NULL WHERE id=? AND status=? "
            "AND (current_execution_id=? OR (current_execution_id IS NULL AND ?=''))"));
        pstmt->setInt(1, TASK_SCHEDULED);
        pstmt->setString(2, id);
        pstmt->setInt(3, TASK_RUNNING);
        pstmt->setString(4, execution_id);
        pstmt->setString(5, execution_id);
        return pstmt->executeUpdate() > 0;
    } catch (sql::SQLException& e) {
        recordSqlError("recoverTaskExecution", e);
        return false;
    }
}
...
bool MySQLClient::updateTaskSchedule(const std::string& id, const std::string& next_run_at,
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> pstmt(lease.conn->prepareStatement(
            "UPDATE tasks SET next_run_at=?, last_run_at=?, retry_count=? "
            "WHERE id=? AND status<>?"));
        set_datetime_or_null(*pstmt, 1, next_run_at);
        set_datetime_or_null(*pstmt, 2, last_run_at);
        pstmt->setInt(3, retry_count);
        pstmt->setString(4, id);
        pstmt->setInt(5, TASK_RUNNING);
        return pstmt->executeUpdate() > 0 || task_exists_and_is_not_running(*lease.conn, id);
    } catch (sql::SQLException &e) {
...
bool MySQLClient::cancelTask(const std::string& id) {
    ...
    try {
        ...
        return task_exists(*lease.conn, id);
    } catch (sql::SQLException &e) {
...
bool MySQLClient::ensureSchema(sql::Connection& conn) {
    try {
        ...
        if (!indexExists(conn, "tasks", "idx_tasks_status_started_at")) executeStatement(conn, "CREATE INDEX idx_tasks_status_started_at ON tasks (status, started_at)");
        return true;
    } catch (sql::SQLException &e) {
